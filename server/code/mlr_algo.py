...
def nutrients(state, rainfall_final, temp_final):
    ...
    try:
        ...
        with open('./code/code/cropDB.csv', 'r') as csvfile, open('./code/code/metacrops.csv', 'w') as metacrops:
            reader = csv.reader(csvfile)
            ...
            for row in reader:
                ...
                if(narea >= ncrop and parea >= pcrop and karea >= kcrop):
                    ...
                    total = row[0]+","+str(rainfall_final[no_months-1]) + \
                        ","+str(temp_final[no_months-1])+","+ph+"\n"
...
def filewrite():
    n = 1
    try:
        with open("./code/code/metacrops.csv", 'r') as f:
...
def regression():
    ...
    locbased = pd.read_csv('./code/code/metacrops.csv')
    ...
    try:
        ...
        sys.exit("No such file exists")
    os.remove('./code/code/metacrops.csv')
    os.remove('./code/code/metacrops11.csv')
#-----------------------------------------------------------------------------#
